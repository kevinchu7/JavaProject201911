...
/**
 * 文件名：LoadServlet。jsp
 * 作用：用于文件（这里主要指excel表）的上传和下载，支持excel表格批量导入试题
 * 创建者：陈天、瞿嘉乐、季洋洋、朱凯
 * 创建时间：
 * 
 */
...
public class LoadServlet extends HttpServlet {
	...
	private void upLoad(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		java.io.File f = new java.io.File("D:\\test\\");
		if(!f.exists()){
			f.mkdirs();
		}
		...
		if(result) {
    		...
    		out.println("<script>window.location.href='/ZijinQuestionBankSystem/teacher/addQuestion.jsp'</script>");
		}
		else
