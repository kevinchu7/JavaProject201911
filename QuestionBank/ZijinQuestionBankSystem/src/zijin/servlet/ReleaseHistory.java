...
/**
 * 文件名：ReleaseHistory.java
 * 作用：teacher查看自己负责的试卷（watch(tid)），或者查看某一张试卷(find(paperid))
 * 创建者：陈天、瞿嘉乐、季洋洋、朱凯
 * 创建时间：
 * 
 */
...
public class ReleaseHistory extends HttpServlet {
	...
	private void findpaperHistory(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		out.println("<script>window.location.href='/ZijinQuestionBankSystem/teacher/releaseHistory.jsp'</script>");
	}
