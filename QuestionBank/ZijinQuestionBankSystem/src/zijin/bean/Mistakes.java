package zijin.bean;

/**
 * 文件名：Mistakes.java
 * 作用：封装具体错题数据信息，侧重于具体描述（哪一张题，错误选项等）-》student.detailMistake
 * 创建者：陈天、瞿嘉乐、季洋洋、朱凯
 * 创建时间：
 * 
 */
public class Mistakes {
	private int mid;
	private int sid;
	private int questiontype;
	private int qid;
	private String misanswer;
	private int paperid;
	private int score;
	private int c_id;
	private String c_choiceA;
    private String c_choiceB;
    private String c_choiceC;
    private String c_choiceD;
    private String c_question;
	private String c_answer;
	private int f_id;
	private String f_question;
	private String f_answer;
	private int j_id;
	private String j_question;
	private String j_answer;
    public int getScore() {
		return score;
	}
	public void setScore(int score) {
		this.score = score;
	}
	public int getC_id() {
		return c_id;
	}
	public void setC_id(int c_id) {
		this.c_id = c_id;
	}
	public String getC_choiceA() {
		return c_choiceA;
	}
	public void setC_choiceA(String c_choiceA) {
		this.c_choiceA = c_choiceA;
	}
	public String getC_choiceB() {
		return c_choiceB;
	}
	public void setC_choiceB(String c_choiceB) {
		this.c_choiceB = c_choiceB;
	}
	public String getC_choiceC() {
		return c_choiceC;
	}
	public void setC_choiceC(String c_choiceC) {
		this.c_choiceC = c_choiceC;
	}
	public String getC_choiceD() {
		return c_choiceD;
	}
	public void setC_choiceD(String c_choiceD) {
		this.c_choiceD = c_choiceD;
	}
	public String getC_question() {
		return c_question;
	}
	public void setC_question(String c_question) {
		this.c_question = c_question;
	}
	public String getC_answer() {
		return c_answer;
	}
	public void setC_answer(String c_answer) {
		this.c_answer = c_answer;
	}
	public int getF_id() {
		return f_id;
	}
	public void setF_id(int f_id) {
		this.f_id = f_id;
	}
	public String getF_question() {
		return f_question;
	}
	public void setF_question(String f_question) {
		this.f_question = f_question;
	}
	public String getF_answer() {
		return f_answer;
	}
	public void setF_answer(String f_answer) {
		this.f_answer = f_answer;
	}
	public int getJ_id() {
		return j_id;
	}
	public void setJ_id(int j_id) {
		this.j_id = j_id;
	}
	public String getJ_question() {
		return j_question;
	}
	public void setJ_question(String j_question) {
		this.j_question = j_question;
	}
	public String getJ_answer() {
		return j_answer;
	}
	public void setJ_answer(String j_answer) {
		this.j_answer = j_answer;
	}
	public int getMid() {
		return mid;
	}
	public void setMid(int mid) {
		this.mid = mid;
	}
	public int getSid() {
		return sid;
	}
	public void setSid(int sid) {
		this.sid = sid;
	}
	public int getQuestiontype() {
		return questiontype;
	}
	public void setQuestiontype(int questiontype) {
		this.questiontype = questiontype;
	}
	public int getQid() {
		return qid;
	}
	public void setQid(int qid) {
		this.qid = qid;
	}
	public String getMisanswer() {
		return misanswer;
	}
	public void setMisanswer(String misanswer) {
		this.misanswer = misanswer;
	}
	public int getPaperid() {
		return paperid;
	}
	public void setPaperid(int paperid) {
		this.paperid = paperid;
	}

}
